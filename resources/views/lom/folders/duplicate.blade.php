        ...
        <div class="container-fluid">
            <div class="card border-0 shadow-sm rounded-lg overflow-hidden">
                <div class="card-header bg-gradient-primary text-white py-3">
                    <h2 class="mb-0 fw-bold" style="font-family: 'Poppins', sans-serif;">Duplikasi Folder untuk {{ $subTopic->title }}</h2>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('folders.storeDuplicate', $folder->id) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                        @csrf
                        <input type="hidden" name="sub_topic_id" value="{{ $subTopic->id }}">
                        <input type="hidden" name="original_folder_id" value="{{ $folder->id }}">
                        ...
                        <div class="row gap-4">
                            <div class="col-12">
                                ...
                                <input type="text" name="name" id="name" class="form-control rounded border-light-subtle @error('name') is-invalid @enderror" value="{{ old('name', $folder->name . ' (Salinan)') }}" required aria-describedby="name-error">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description" id="description" rows="4" class="form-control rounded border-light-subtle @error('description') is-invalid @enderror">{{ old('description', $folder->description . ' (Salinan)') }}</textarea>
                            ...
                            @php
                                ...
                                $selectedDimensionId = $folder->options->first() ? $folder->options->first()->pivot->dimension_id : null;
                            @endphp
                            <div class="col-12">
                            ...
                            <div class="col-12 mt-4">
                                <div class="d-flex justify-content-between">
                                    <button type="submit" class="btn btn-primary px-4 py-2 fw-medium" style="font-family: 'Poppins', sans-serif; background-color: #3699FF; border-color: #3699FF;" name="action" value="save_return">
                                        Simpan dan Kembali ke Kursus
                                    </button>
                                    <button type="submit" class="btn btn-success px-4 py-2 fw-medium" style="font-family: 'Poppins', sans-serif; background-color: #28a745; border-color: #28a745;" name="action" value="save_display">
                                        Simpan dan Tampilkan
                                    </button>
                                    <a href="{{ route('sections.show', [$subTopic->section->course_id, $subTopic->section->id]) }}" class="btn btn-outline-secondary px-4 py-2 fw-medium" style="font-family: 'Poppins', sans-serif;">
                                        Batal
                                    </a>
                                </div>
                            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (typeof tinymce !== 'undefined') {
                ...
                console.error('TinyMCE tidak dimuat! Pastikan CDN dimuat di layout atau tambahkan <script src="https://cdn.tiny.cloud/1/no-api-key/tinymce/5/tinymce.min.js" referrerpolicy="origin"></script> di view.');
            }
        });
    </script>
