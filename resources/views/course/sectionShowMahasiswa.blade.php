                                ...
                                <button class="accordion-button bg-light" type="button" data-bs-toggle="collapse"
                                    ...
                                    <div class="d-flex justify-content-between align-items-center w-100">
                                        ...
                                        <div class="form-check me-3">
                                            @php
                                                $hasQuizzes = $section->sub_topic[0]->quizs->count() > 0;
                                                $allQuizzesAttempted = !$hasQuizzes || $section->sub_topic[0]->quizs->every(function ($quiz) {
                                                    return $quiz->attempts()->where('user_id', Auth::id())->exists();
                                                });
                                            @endphp
                                            <input class="form-check-input subtopic-checkbox" type="checkbox"
                                                ...
                                                {{ $allQuizzesAttempted && $hasQuizzes ? '' : 'disabled' }}>
                                ...
                                <div class="accordion-header bg-white rounded shadow-sm">
                                    <button class="accordion-button collapsed bg-light" type="button" data-bs-toggle="collapse"
                                        ...
                                        <div class="d-flex justify-content-between align-items-center w-100">
                                            ...
                                            <div class="form-check me-3">
                                                @php
                                                    $hasQuizzes = $section->sub_topic[$i]->quizs->count() > 0;
                                                    $allQuizzesAttempted = !$hasQuizzes || $section->sub_topic[$i]->quizs->every(function ($quiz) {
                                                        return $quiz->attempts()->where('user_id', Auth::id())->exists();
                                                    });
                                                @endphp
                                                <input class="form-check-input subtopic-checkbox" type="checkbox"
                                                        ...
                                                        @foreach($section->sub_topic[$i]->infografis as $infografis)
                                                            <div class="list-group-item list-group-item-action border-0 rounded shadow-sm mb-2">
                                                                <a href="{{ asset('storage/' . $infografis->file_path) }}" target="_blank"
                                                                   class="text-decoration-none d-flex align-items-center">
                                                                    <i class="fas fa-chart-pie me-3 text-success fs-4"></i>
                                                                    <div>
                                                                        <h6 class="mb-0">{{ $infografis->name }}</h6>
                                                                        <small class="text-muted">Infographic</small>
                                                                    </div>
                                                                </a>
                                                            </div>
                                                        @endforeach
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function checkSubtopicQuizzesCompleted(subtopicId) {
            const checkbox = document.querySelector(`#subtopic-check-${subtopicId}`);
        ...
        setInterval(() => {
            ...
            visibleSubtopics.forEach(subtopic => {
                ...
                const checkbox = subtopic.querySelector('.subtopic-checkbox');
                const quizItems = subtopic.querySelectorAll('.list-group-item');
                let hasQuizzes = false;
                let allQuizzesCompleted = true;

                // Check if there are any quizzes in this subtopic
                const quizElements = subtopic.querySelectorAll('.fa-question-circle');
                hasQuizzes = quizElements.length > 0;

                if (hasQuizzes) {
                    quizItems.forEach(item => {
                        if (item.querySelector('.fa-question-circle')) {
                            const completedBadge = item.querySelector('.badge.bg-success');
                    ...
                    // If no quizzes, disable the checkbox
                    allQuizzesCompleted = false;
                }

                if (hasQuizzes && allQuizzesCompleted) {
                    checkbox.disabled = false;
                    checkbox.parentElement.querySelector('label').style.opacity = '1';
                } else {
...
<style>
    ...
    .subtopic-checkbox:disabled {
        cursor: not-allowed;
    }
    .subtopic-checkbox:disabled + label {
        opacity: 0.5;
    }
    .animate__fadeIn {
