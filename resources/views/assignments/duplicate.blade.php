...
@section('content')
    ...
    <div class="container mt-4">
        <h2>Duplikasi Assignment for {{ $subTopic->title }}</h2>
        <form method="POST" action="{{ route('assignments.storeDuplicate', $assignment->id) }}" class="needs-validation" novalidate enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="sub_topic_id" value="{{ $subTopic->id }}">
            <input type="hidden" name="original_assignment_id" value="{{ $assignment->id }}">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                               value="{{ old('name', $assignment->name . ' (Salinan)') }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $assignment->description . ' (Salinan)') }}</textarea>
                        @error('description')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    @php
                        ...
                        $selectedOption = $assignment->dimension_option_id ?? old('dimension_options', $assignment->options->pluck('id')->first());
                    @endphp
