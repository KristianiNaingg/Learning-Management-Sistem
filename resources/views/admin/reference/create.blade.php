<div class="container">
    <h2>Tambah Referensi untuk {{ $topic->title }}</h2>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form id="createReferenceForm" action="{{ route('topics.reference.store', [$course->id, $topic->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<script>
    document.getElementById('createReferenceForm').addEventListener('submit', function(e) {
        ...
        .catch(error => {
            ...
            alert(error.message || 'Gagal menyimpan referensi. Silakan coba lagi.');
            if (error.response) {
