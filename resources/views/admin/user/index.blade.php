                ...
                <div class="col-12">
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered" id="tableUser">
                            ...
                            {{-- Data will be automatically filled by DataTables --}}
                            </tbody>
                        </table>
                    </div>
                </div>
        ...
        $(document).ready(function () {
            ...
            $('body').on('click', '.editUser', function () {
                ...
                $.get("{{ route('users.edit', ':id') }}".replace(':id', user_id), function (data) {
                    ...
                    $('#password').val('********'); // Set masked placeholder
                    $('#password').attr('placeholder', 'Enter new password');
                    $('#password').data('original', '********'); // Store placeholder for comparison
                }).fail(function (xhr) {
                    console.error("Error fetching user data:", xhr);
                    swal_error('Failed to retrieve user data');
                });
            });
            ...
            $('#saveBtn').click(function (e) {
                ...
                if (!user_id) {
                    if (!id) {
                        swal_error('NRP/NIP/NIK must be filled!');
                        $(this).html('Save changes');
                        return;
                    }
                    if (!password || password.length < 5) {
                        swal_error('Password must be at least 5 characters for new users!');
                        $(this).html('Save changes');
                        return;
                    }
                }

                // Skip password if it's the placeholder during edit
                if (user_id && password === '********') {
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        ...
                        if (xhr.status === 422 && xhr.responseJSON.message === 'The id has already been taken.') {
                            errorMsg = 'NRP/NIP/NIK has already been used!';
                        }
                        swal_error(errorMsg);
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function (xhr) {
                                var errorMsg = xhr.responseJSON && xhr.responseJSON.error ? xhr.responseJSON.error : 'Failed to delete user';
                                swal_error(errorMsg);
                            }
                        });
                    }
