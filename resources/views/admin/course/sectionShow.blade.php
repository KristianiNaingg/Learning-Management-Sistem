...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Modified function to create tools for items (label, file, infografis, etc.)
        function createItemTools(container, itemId, type, editRoute, deleteRoute, duplicateRoute) {
            const toolsDiv = document.createElement('div');
            toolsDiv.className = 'd-flex align-items-center gap-2';
            ...
            // Duplicate Button (excluded for Quiz)
            if (type !== 'Quiz' && duplicateRoute) {
                const duplicateButton = document.createElement('a');
                duplicateButton.className = 'btn btn-sm btn-outline-primary rounded-circle p-2';
                duplicateButton.setAttribute('title', `Duplicate ${type}`);
                duplicateButton.setAttribute('data-bs-toggle', 'tooltip');
                duplicateButton.href = duplicateRoute.replace('ITEM_ID', itemId);
                duplicateButton.innerHTML = '<i class="fas fa-copy"></i>';
                toolsDiv.appendChild(duplicateButton);
            }
                ...
                const toolDivs = [
                    {
                        divs: labelToolDivs,
                        type: 'Label',
                        editRoute: '/labels/ITEM_ID/edit',
                        deleteRoute: '/labels/ITEM_ID',
                        duplicateRoute: '/labels/ITEM_ID/duplicate'
                    },
                    {
                        divs: fileToolDivs,
                        type: 'File',
                        editRoute: '/files/ITEM_ID/edit',
                        deleteRoute: '/files/ITEM_ID',
                        duplicateRoute: '/files/ITEM_ID/duplicate'
                    },
                    {
                        divs: infografisToolDivs,
                        type: 'Infografis',
                        editRoute: '/infografis/ITEM_ID/edit',
                        deleteRoute: '/infografis/ITEM_ID',
                        duplicateRoute: '/infografis/ITEM_ID/duplicate'
                    },
                    {
                        divs: assignmentToolDivs,
                        type: 'Assignment',
                        editRoute: '/assignments/ITEM_ID/edit',
                        deleteRoute: '/assignments/ITEM_ID',
                        duplicateRoute: '/assignments/ITEM_ID/duplicate'
                    },
                    {
                        divs: forumToolDivs,
                        type: 'Forum',
                        editRoute: '/forums/ITEM_ID/edit',
                        deleteRoute: '/forums/ITEM_ID',
                        duplicateRoute: '/forums/ITEM_ID/duplicate'
                    },
                    {
                        divs: lessonToolDivs,
                        type: 'Lesson',
                        editRoute: '/lessons/ITEM_ID/edit',
                        deleteRoute: '/lessons/ITEM_ID',
                        duplicateRoute: '/lessons/ITEM_ID/duplicate'
                    },
                    {
                        divs: urlToolDivs,
                        type: 'URL',
                        editRoute: '/urls/ITEM_ID/edit',
                        deleteRoute: '/urls/ITEM_ID',
                        duplicateRoute: '/urls/ITEM_ID/duplicate'
                    },
                    {
                        divs: folderToolDivs,
                        type: 'Folder',
                        editRoute: '/folders/ITEM_ID/edit',
                        deleteRoute: '/folders/ITEM_ID',
                        duplicateRoute: '/folders/ITEM_ID/duplicate'
                    },
                    {
                        divs: pageToolDivs,
                        type: 'Page',
                        editRoute: '/pages/ITEM_ID/edit',
                        deleteRoute: '/pages/ITEM_ID',
                        duplicateRoute: '/pages/ITEM_ID/duplicate'
                    },
                    {
                        divs: quizToolDivs,
                        type: 'Quiz',
                        editRoute: '/quizs/ITEM_ID/edit',
                        deleteRoute: '/quizs/ITEM_ID',
                        duplicateRoute: ''
                    },
                    {
                        divs: referenceToolDivs,
                        type: 'Reference',
                        editRoute: '{{ route("sections.referensi.edit", [$course->id, $section->id, "ITEM_ID"]) }}',
                        deleteRoute: '/referensi/ITEM_ID',
                        duplicateRoute: '/referensi/ITEM_ID/duplicate'
                    }
                ];

                toolDivs.forEach(({ divs, type, editRoute, deleteRoute, duplicateRoute }) => {
                    divs.forEach(div => {
                        if (isConstructionMode) {
                            createItemTools(div, div.id.replace(`${type.toLowerCase()}Tools`, ''), type, editRoute, deleteRoute, duplicateRoute);
                        } else {
                            removeTools(div);
                        }
                    });
