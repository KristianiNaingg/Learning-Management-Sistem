                    ...
                    <form action="{{ route('courses.update', $course->id) }}" method="POST" id="courseForm" enctype="multipart/form-data">
                        ...
                        <div id="general-section">
                            ...
                            <div class="d-flex justify-content-between gap-2">
                                <a href="{{ route('admin.courses.index') }}" class="btn btn-outline-secondary w-25">Cancel</a>
    ...
    <script>
        ...
        $("#participant_input").autocomplete({
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        response(data.map(user => ({
                            label: `${user.id}-${user.name}`, // Display as Id-nama
                            value: user.name, // Input field will show name only
                            id: user.id,
                            name: user.name
                        })));
                    },
            ...
            select: function (event, ui) {
                ...
                $("#participant_input").val(ui.item.name); // Set input to name only
                return false; // Prevent default behavior of setting label
            }
        });
        ...
        function addParticipant() {
            ...
            if (!participantId || !participantName) {
                alert('Please select a valid user from the autocomplete suggestions.');
                return;
            }
        ...
        function updateParticipantList() {
            ...
            if (participants.length === 0) {
                ...
                participants.forEach((participant, index) => {
                    ...
                    li.innerHTML = `
                        ${participant.id}-${participant.name} (${participant.role.charAt(0).toUpperCase() + participant.role.slice(1)})
        ...
        function populateReviewSection() {
            ...
            if (participants.length === 0) {
                ...
                participants.forEach(participant => {
                    ...
                    li.textContent = `${participant.id}-${participant.name} (${participant.role.charAt(0).toUpperCase() + participant.role.slice(1)})`;
                    ul.appendChild(li);
                });
