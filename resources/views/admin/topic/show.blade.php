    ...
    <div class="container">
        ...
        <div class="card mb-5 border-0 shadow-sm">
            <div class="card-header bg-gradient-secondary text-white py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h2 class="mb-0 fs-3 fw-bold"><i class="fas fa-book-open me-2"></i>{{ $topic->title }}</h2>
                    <span class="badge bg-white text-secondary fs-6">Topic</span>
                </div>
            </div>
            <div class="card-body bg-light">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="bg-white p-3 rounded shadow-sm h-100">
                            ...
                            <p class="card-text text-muted">{{ $topic->sub_cpmk ?? 'No sub-CPMK available' }}</p>
                        </div>
                    </div>
                </div>
            </div>
                                            ...
                                            <div class="resources-container mb-4">
                                                ...
                                                <div class="list-group">
                                                    ...
                                                    <div class="collapse show" id="inputDimension{{ $subTopic->id }}">
                                                        @foreach($subTopic->sorted_items as $item)
                                                            @if(in_array(get_class($item), [
                                                                'App\Models\LomFile',
                                                                'App\Models\LomFolder',
                                                                'App\Models\LomUrl',
                                                                'App\Models\LomInfographic',
                                                                'App\Models\LomLabel'
                                                            ])&& $item->learningStyleOption && $item->learningStyleOption->dimension && $item->learningStyleOption->dimension->dimension === 'Input')
                                                    ...
                                                    <div class="collapse show" id="perceptionDimension{{ $subTopic->id }}">
                                                        @foreach($subTopic->sorted_items as $item)
                                                            @if(in_array(get_class($item), [
                                                                ...
                                                                'App\Models\LomFolder',
                                                                'App\Models\LomUrl'
                                                           ]) && $item->learningStyleOption && $item->learningStyleOption->dimension && $item->learningStyleOption->dimension->dimension === 'Perception')
                                                        @include('partials.resource_item', ['item' => $item])
                                                    @endif
                                                        @endforeach
                                                    </div>
                                                    ...
                                                    <div class="collapse show" id="processingDimension{{ $subTopic->id }}">
                                                        @foreach($subTopic->sorted_items as $item)
                                                            @if(in_array(get_class($item), [
                                                                'App\Models\LomLesson',
                                                                'App\Models\LomForum',
                                                                'App\Models\LomAssign',
                                                                'App\Models\LomQuiz'
                                                            ]))
                                                                @include('partials.resource_item', ['item' => $item])
                                                            @endif
                                            ...
                                            <div class="tab-content p-3" id="myTabContent{{ $subTopic->id }}">
                                                ...
                                                <div class="tab-pane fade" id="knowledge{{ $subTopic->id }}" role="tabpanel" aria-labelledby="knowledge-tab{{ $subTopic->id }}">
                                                    <div class="row g-3">
                                                        ...
                                                        <div class="col-md-4">
                                                            <a href="{{ route('urls.create', ['sub_topic_id' => $subTopic->id]) }}" class="btn btn-outline-danger w-100 h-100 d-flex flex-column align-items-center py-3">
                                                        ...
                                                        <div class="col-md-4">
                                                            <a href="{{ route('infographics.create', ['sub_topic_id' => $subTopic->id]) }}" class="btn btn-outline-success w-100 h-100 d-flex flex-column align-items-center py-3">
        ...
        @if(count($topic->references) > 0)
            <div class="card mb-5 border-0 shadow-sm">
                ...
                <div class="card-body">
                    <div class="list-group">
                        @foreach($topic->references as $reference)
                            <div class="list-group-item list-group-item-action border-0 rounded shadow-sm mb-2">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <p class="mb-0">{{ $reference->content ?? 'No content available' }}</p>
                                    </div>
                                    <div id="referenceTools{{ $reference->id }}" class="d-flex align-items-center gap-1"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="d-flex justify-content-between mt-4 mb-5">
            ...
            @can('update', $topic)
                <a href="{{ route('topics.edit', [$course->id, $topic->id]) }}" class="btn btn-primary rounded-pill px-4 py-2">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (addSubTopicButton) {
            addSubTopicButton.setAttribute('onclick', `window.location.href='{{ route('topics.subtopic.create', ['course' => $course->id, 'topic_id' => $topic->id]) }}'`);
        }

        // Set the onclick handler for the Add Reference button
        if (addReferenceButton) {
            addReferenceButton.setAttribute('onclick', `window.location.href='{{ route('topics.reference.create', ['course' => $course->id, 'topic_id' => $topic->id]) }}'`);
        }
        ...
        function createConstructionTools(container, subTopicId) {
            ...
            editButton.href = "{{ route('topics.subtopic.edit', [$course->id, $topic->id, ':subTopicId']) }}".replace(':subTopicId', subTopicId);
            ...
            deleteButton.addEventListener('click', function () {
                if (confirm('Are you sure you want to delete this subtopic?')) {
                    fetch("{{ route('topics.subtopics.destroy', [$course->id, $topic->id, ':subTopicId']) }}".replace(':subTopicId', subTopicId), {
                        method: 'DELETE',
                        headers: {
        ...
        // Function to create tools for references
        function createReferenceTools(container, referenceId) {
            const toolsDiv = document.createElement('div');
            toolsDiv.className = 'd-flex align-items-center gap-2';

            // Edit Button
            const editButton = document.createElement('a');
            editButton.className = 'btn btn-sm btn-outline-success rounded-circle p-2';
            editButton.setAttribute('title', 'Edit Reference');
            editButton.setAttribute('data-bs-toggle', 'tooltip');
            editButton.href = "{{ route('topics.reference.edit', [$course->id, $topic->id, ':referenceId']) }}".replace(':referenceId', referenceId);
            editButton.innerHTML = '<i class="fas fa-edit"></i>';
            toolsDiv.appendChild(editButton);

            // Delete Button
            const deleteButton = document.createElement('button');
            deleteButton.className = 'btn btn-sm btn-outline-danger rounded-circle p-2';
            deleteButton.setAttribute('title', 'Delete Reference');
            deleteButton.setAttribute('data-bs-toggle', 'tooltip');
            deleteButton.innerHTML = '<i class="fas fa-trash"></i>';
            deleteButton.addEventListener('click', function () {
                if (confirm('Are you sure you want to delete this reference?')) {
                    fetch("{{ route('topics.references.destroy', [$course->id, $topic->id, ':referenceId']) }}".replace(':referenceId', referenceId), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json'
        ...
        if (modeButton) {
            modeButton.addEventListener('click', function () {
                ...
                // Toggle tools for references
                referenceToolDivs.forEach(div => {
                    if (isConstructionMode) {
                        createReferenceTools(div, div.id.replace('referenceTools', ''));
                    } else {
                        removeTools(div);
                    }
                });
