...
{{--        @include('components.mode_konstruksi')--}}
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                <div class="row">
                    @if(!$course->topics || $course->topics->isEmpty())
                                            ...
                                            <h6 class="fw-bold">
                                                @if(Auth::check())
                                                    @if(Auth::user()->id_role == 2 || Auth::user()->id_role == 1)
                                                        <a href="{{ route('student.topic.show', ['course_id' => $course->id, 'topic_id' => $topic->id]) }}">{{ $topic->title }}</a>
                                                    @else
                                                            <a href="{{ route('student.topic.show', ['course_id' => $course->id, 'topic_id' => $topic->id]) }}">{{ $topic->title }}</a>                                                    @endif
                                                @else
